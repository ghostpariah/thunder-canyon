...
window.onload = () => {
    createComponent(
        document.getElementById("dateInWrapper"),
        "date in",
        null,
        "DateIn",
        "edit"
    ...
    createComponent(
        document.getElementById("originWrapper"),
        "comboBox",
        originList,
        "Designation",
        "edit"
    ...
    createComponent(
        document.getElementById("dateSchWrapper"),
        "date sched",
        null,
        "DateSched",
        "edit"
    ...
    if (!document.getElementById("customerNames")) {
        createComponent(
            document.getElementById("customerComboBoxContainer"),
            "comboBox",
    ...
    createComponent(
        document.getElementById("sbContacts"),
        "split select",
        null,
        "Contacts",
        "edit"
    ...
    createComponent(
        document.getElementById("unitTypeWrapper"),
        "textBox",
        null,
        "UnitType",
        "edit"
    ...
    createComponent(
        document.getElementById("jobTypeWrapper"),
        "comboBox",
...
document.addEventListener("click", (event) => {
    ...
    if (event.target.classList != "") {
        if (
            !event.target.classList.contains("listItem") &&
            !event.target.parentNode.classList.contains("selectBox") &&
            !event.target.parentNode.classList.contains("comboBox")
        ) {
            closeDropDowns();
        } else {
            if (
    ...
    if (
        event.target.id != "DateSched-choice" &&
        event.target.id != "btn-DateSched"
    ) {
        ...
        document
            .getElementById("DateSched-choice")
    ...
    $(selDesignation).on({
        change: function (event) {
            switch (this.options[this.selectedIndex].text) {
                case "Scheduled":
                    document.getElementById("dateWrapperSch").style.display =
                        "block";
                    document.getElementById("OTL_wrapper").style.display =
                        "none";
                    document.getElementById(
                        "dateWrapper_OTL_SCHEDULED"
                    ).style.display = "none";
...
function loadData(objJobToEdit) {
    ...
    let customerNamesList = Array.from(
        document.querySelectorAll("#Customer-listBox .listItem")
    );
    ...
    switch (d.designation) {
        ...
        case "Scheduled":
            ...
            document.getElementById("dateSchWrapper").className =
                "visibleInput";
            break;
        case "Coming - No Appt":
            document.getElementById("dateSchWrapper").className =
    ...
    d.date_scheduled != null
        ? (inpScheduledDate.value = d.date_scheduled)
        : (inpScheduledDate.value = "");
    ...
    console.log(d.time_of_day);
    switch (d.time_of_day) {
        case "am":
            radAM.checked = true;
            radPM.checked = false;
            for (var i = 0, r = radios, l = r.length; i < l; i++) {
                console.log("radio disabled =" + r[i].getAttribute("disabled"));
                if (
                    r[i].getAttribute("disabled") != "" ||
                    r[i].getAttribute("disabled") != null
                ) {
                    r[i].disabled = false;
                }
            }
    ...
    if (d.comeback_customer != null) {
        if (d.comeback_customer == 1) {
            ...
            document.getElementById("dateWrapper_OTL_SCHEDULED").style.display =
                "block";
            if (d.date_scheduled != null) {
                createComponent(
                    document.getElementById("dateWrapper_OTL_SCHEDULED"),
                    "date OTL",
                    null,
                    "DateOTL",
                    "edit"
                ...
                document.getElementById("DateOTL-choice").value =
                    d.date_scheduled;
                d.time_of_day == "am"
                    ? (document.getElementById("radAM_OTL").checked = true)
                    : (document.getElementById("radAM_OTL").checked = false);
                d.time_of_day == "pm"
                    ? (document.getElementById("radPM_OTL").checked = true)
                    : (document.getElementById("radPM_OTL").checked = false);
            }
        }
    }
    d.waiting_customer != null
        ? d.waiting_customer == 1
            ? (cbWaiting.checked = true)
            : (cbWaiting.checked = false)
        : (cbWaiting.checked = false);
    ...
    let contactsList = Array.from(
        document.querySelectorAll("#Contacts-listBox .option")
    );
        ...
        contactsList.forEach((item) => {
            if (item.getAttribute("method-id") == d.number_ID) {
                ...
                document.querySelector("#Contacts-info").innerHTML =
                    item.parentNode.firstChild.innerText;
            }
        });
    }
...
function changeContact(choice) {
    ...
    switch (con_ops) {
        ...
        case "+ add number":
            contactName =
                contactsInput.options[
                    contactsInput.selectedIndex
                ].parentElement.label.split(" ");
            index = contactsInput.options[contactsInput.selectedIndex].index;
            conMeth = "phone";
            con_ID = Number(
                contactsInput.options[
                    contactsInput.selectedIndex
                ].parentElement.getAttribute("contactID")
            );
            ipc.send(
                "open-contacts",
                "edit page",
    ...
    switch (item) {
        ...
        default:
            ...
            for (i = 0; i < arrOptions.length; i++) {
                if (arrOptions.options[i].text.includes("~")) {
                    //startsWith(group)
                    arrOptions.options[i].text = arrOptions.options[
                        i
                    ].text.substring(
                        arrOptions.options[i].text.indexOf("~") + 1
                    );
                }
            }
...
function updateJob() {
    ...
    if (!verified[0]) {
        for (i = 0; i < verified[1].length; i++) {
            ...
            if (type[0] == "Unit") {
                ...
                verified[1][i].parentNode.parentNode.appendChild(
                    createMessageBox(type[0], null)
                );
                document
                    .querySelector("#unitTypeWrapper")
                    .appendChild(createMessageBox("UnitType", null));
            } else {
                if ($(`#${type[0]}-MessageContainer`)) {
                    $(`#${type[0]}-MessageContainer`).remove();
                }
    ...
    if (editData.time_of_day != null && editData.time_of_day != "") {
        editData.time_of_day.localeCompare(
            $("input[name=ampmSched]:checked").val()
        ) != 0
            ? (objNewJob.time_of_day =
    ...
    if (editData.date_scheduled?.localeCompare(dateScheduled.value) != 0) {
        if (
            ...
            objNewJob.julian_date = jDate(
                document.getElementById("DateSched-choice").value
            );
        }
    }
    ...
    if (Boolean(cbOTL.checked) != Boolean(editData.comeback_customer)) {
        if (cbOTL.checked) {
            if (
                datepickerOTL.value == "" ||
                datepickerOTL.value == undefined ||
                datepickerOTL.value == null
            ) {
                document.getElementById("wrapperOTL").style.display = "block";
                document.getElementById("OTL_message").innerHTML =
            ...
            objNewJob.time_of_day =
                $("input[type=radio]:checked").size() > 0
                    ? $("input[name=ampmOTL]:checked").val()
                    : "z";
            objNewJob.julian_date = jDate(
                document.getElementById("DateOTL-choice").value
            );
...
function openOTLandScheduled(event, cb, OTL_container) {
    ...
    if (cb.checked == true) {
        ...
        if (
            alreadyScheduled ||
            selDesignation.innerText == "Scheduled" ||
            selDesignation.innerText == "Coming - No Appt"
        ) {
            $("#Designation0").mousedown();
            $(cb).focus();
            // document.getElementById('dateSchWrapper').className = 'hiddenInput'
...
function openInput(e, active, inputID1, inputID2) {
    ...
    if (active.id == "cbOTL_scheduled") {
        ...
        if (inputID2) {
            ...
            switch (choice) {
                case "Scheduled":
                    document.getElementById(inputID2).className =
                        "visibleInput";
                    document.getElementById("OTL_SCHEDULED").className =
                        "hiddenInput";
                    document.getElementById(
                        "dateWrapper_OTL_SCHEDULED"
                    ...
                    break;
                case "On the Lot":
                    document.getElementById("dateWrapper").className =
                        "hiddenInput";
                    document.getElementById("OTL_SCHEDULED").className =
                        "visibleInput";
                ...
                default:
                    document.getElementById("cbWrapper").className =
                        "visibleFieldset";
                    document.getElementById("formButtons").className =
                        "visibleInput";
