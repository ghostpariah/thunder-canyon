...
async function readyApp() {
    ...
    try {
            ...
            setTimeout(() => {
                ...
                fs.watch(white_board, (event, filename) => {
                    if (filename) {
                        ...
                        errLog.error(
                            `whiteboard file doesn't exist or connection to v: lost`
                        );
                    }
                });
                //createMainWindow()
            }, 2000);
            await readyUpdates().catch((e) => {
                errLog.info(e);
            });
        ...
        await checkForDB(workflowDB).catch((e) => {
...
app.on("ready", () => {
    ...
    checkDate = setInterval(function () {
        ...
        appStartDay == currentDay
            ? console.log(`app opened today ${currentDay}`)
            : restartApp();
    }, 3600000);
    readyApp();
    // setTimeout(() => {
    ...
    try {
        fs.readdir(logLocation, function (err, data) {
            if (err) {
                ...
                data.forEach((file) => {
                    ...
                    if (file.length > 4) {
                        ...
                        console.log("day of year after slice = " + doy);
                            ...
                            switch (file.slice(0, 1)) {
                                case "a":
                                    ...
                                    if (doy == "365" || doy == "366") {
                                        ...
                                        newALogPath = path.join(
                                            logLocation,
                                            `${y - 1}/${doy}/activityLog.txt`
                                        );
                                    } else {
                                        if (
                                            !fs.existsSync(
                                ...
                                case "e":
                                    ...
                                    if (doy == lastDay) {
                                        ...
                                        newELogPath = path.join(
                                            logLocation,
                                            `${y - 1}/${doy}/errorLog.txt`
                                        );
                                    } else {
                                        if (
                                            !fs.existsSync(
            ...
            let actLogWatcher = fs.watch(activityLog, (eventType, filename) => {
                console.log(
                    "myAction before fswatch processing is " + myAction
                );
                if (filename) {
                    if (fsWait) return;
                    fsWait = setTimeout(() => {
                    ...
                    errLog.error(
                        `activityLog file doesn't exist or connection to v: lost`
                    );
                }
                console.log("hello");
            });
...
ipcMain.on("restore-database", (event, restorePoint) => {
    ...
    renameDB(workflowDB, corruptedDB)
        .then((res) => {
    ...
    try {
        ...
        setTimeout(() => {
            let restore = dboRestore.backup(
                path.join(dataFolder, "workflow_app.db"),
                (err) => {
                    if (err) {
                        ...
                        let h = setInterval(() => {
                            if (restore.idle) {
                                ...
                                console.log(
                                    "page count = " + restore.pageCount
                                );
                                console.log(
                                    "pages remaining = " + restore.remaining
                                );
                            }
...
ipcMain.on("edit-location-drop", (event, args, args2, args3) => {
    ...
    dboLocation.serialize(() => {
        ...
        console.log("sql string =" + columnPlaceholders);
        let s = `UPDATE jobs SET ${columnPlaceholders} WHERE job_ID=${id}`;
        console.log(s);
        ...
        let sql1 = `UPDATE jobs
                ...
                status = ?
                WHERE job_ID = ?`;
        let sql2 = `SELECT * FROM jobs WHERE job_ID = ${id}`;
        dboLocation
            .run(s, v, function (err) {
                if (err) {
                    return console.error(err.message);
                }
                args.user = args2;
...
ipcMain.on("edit-customer-name", (event, args1, args2) => {
    console.log(
        `args1 in edit-customer-name is ${args1} and args2 is ${args2}`
    );
...
let getCustomerName = async (args) => {
    ...
    async function getIdByName() {
        ...
        let sql = `SELECT customer_name FROM customers WHERE customer_ID = ${Number(
            args
        )}`;
...
function sendStatusToWindow(text, val) {
    if (updateWin) {
        val
            ? updateWin.webContents.send("updater", text, val)
...
ipcMain.on("create-user", (event, args) => {
    ...
    let sql =
        "INSERT INTO users(user_name,role,active,password) VALUES(?,?,?,?)";
    let sql2 = `SELECT *FROM users`;
    dboUser.serialize(() => {
        dboUser
            .run(sql, data, function (err) {
                if (err) {
...
ipcMain.on("add-job", (event, args, args2, args3) => {
    ...
    dboAddJob.run(sql, v, function (err) {
        if (err) {
            ...
            console.log(
                `${this.changes} items inserted at row: ${this.lastID}`
            );
        }
    });
});
...
ipcMain.on("db-contact-add", (event, args) => {
    ...
    dboContacts.serialize(function () {
        ...
        dboContacts.run(sql, v, function (err) {
            ...
            console.log(
                `${this.changes} items inserted at row: ${this.lastID} of contacts`
            );
            ...
            if (args.number != null) {
                ...
                v = 0;
                sql =
                    "SELECT * FROM phone_numbers WHERE phone_ID=? AND active=1";
            }
            dboContacts.run(sql, v, function (err) {
                if (err) {
                    console.log(err.message);
                ...
                item_ID = this.lastID;
                console.log(
                    `${this.changes} items inserted at row: ${this.lastID} of phone_numbers`
                );
                if (args.email != null) {
                ...
                dboContacts.run(sql, v, function (err) {
                    ...
                    if (method_count > 1) {
                        ...
                        item_ID = this.lastID;
                        console.log(
                            `${this.changes} items inserted at row: ${this.lastID} of emails`
                        );
                        event.returnValue = item_ID;
                    }
                });
...
function createMainWindow() {
    ...
    console.log(
        "main window has loaded this is just before the session is create"
    );
...
ipcMain.on("open-contacts-add", (event, props) => {
    ...
    console.log(props);
    getCustomerName(props.customer_ID).then((resolve) => {
    ...
    (event, args1, args2, args3, args4, args5, args6, args7, args8) => {
        ...
        if (args1 == "add job page" && !args6) {
            ...
            dboCustomers.all(sql, args2.toUpperCase(), function (err, row) {
                ...
                console.log(
                    `has args onw and args 6. returns ${row[0].customer_ID}`
                );
    ...
    try {
        ...
        if (args3 === true) {
            list = fs.readFileSync(
                `${logLocation}activityLog${args2}.txt`,
                "UTF-8"
            );
        } else {
            list = fs.readFileSync(
                `${logLocation}${args1}\\${args2}\\activityLog.txt`,
                "UTF-8"
            );
        }
...
async function backupDB() {
    ...
    if (objStats) {
        ...
        let date = new Date();
        alreadyBackedUp =
            objStats.mtime.getDate() == date.getDate() ? true : false;
