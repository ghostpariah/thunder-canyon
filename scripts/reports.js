...
function getYesterday() {
    ...
    if (newMonth) {
        return (
            ...
            1 +
            "/" +
            getDaysInMonth(todaysYear, todaysMonth - 1) +
            "/" +
            todaysYear
        );
    }
    ...
    printPDFBtn.addEventListener("click", function (event) {
        ...
        if (totalACH > 0) {
            ...
            for (i = 1; i <= validCount; i++) {
                ...
                eodInfo.ach[i - 1] = {};
                eodInfo.ach[i - 1].amount = document.getElementById(
                    `ia${i}`
                ).value;
                console.log("after i=" + i);
                eodInfo.ach[i - 1].customer = document.getElementById(
                    `if${i}`
                ).value;
            }
        ...
        document
            .getElementById("ribbon4")
            .setAttribute("class", "ribbon hidden");
        ...
        let tot =
            Number(c.replace(/[^0-9.\-]/g, "")) +
            Number(d.replace(/[^0-9.\-]/g, ""));

        let t = document.createTextNode(
            `$${tot.toFixed(2).replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}`
        );
        ...
        keyup: (event) => {
            if (
                ...
                if (num.length == 8) {
                    ...
                    if (
                        event.key != "0" &&
                        Number(event.target.value.substring(6, 7)) >= 2
                    ) {
...
function toggleVisibility(el) {
    ...
    for (i = 1; i < mainOpts.length + 1; i++) {
        if (mainOpts.id != "ribbon5") {
            document
                ...
                .setAttribute("class", "mainOption unchosen");
            document
    ...
    if (chosenOption == 1) {
        ...
        ribVisibility < 0
            ...
            : ribbon.setAttribute("class", "ribbon visible flexCenter");
        displayLotReport();
        // }, 300);
    }
    if (chosenOption == 4) {
...
function search(searchType) {
    ...
    searchData =
        searchCriteria.value !== ""
            ? filterItems(objSearch, searchCriteria.value)
            : "";
...
function createEODitem(el, item, count, achAmount) {
    console.log(
        "beginning of createEODitem..item= " + item + "count= " + count
    );
    ...
    if (alreadyExists) {
        if (item != "date" && item != "from") {
            if (item == "ach") {
                ...
                $(`#${item + count} :nth-child(2)`).html(
                    resetText(el, item + count, count, achAmount)[0]
                ); //resetText(el,item)
                $(`#${item + count} :nth-child(1)`).html(
                    resetText(el, item + count, count, achAmount)[1]
                );
            } else {
    ...
    switch (item) {
        case "date":
            text = document.createTextNode(
                `END OF DAY ${getYesterday(
                    $("#datepickerReport").datepicker("getDate")
                )}`
            );
        ...
        case "ach":
            ...
            resultText = document.createTextNode(
                `${formatToCurrency(achAmount).replace(
                    /\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g,
                    ","
                )}`
            ); //el.parentNode.childNodes[1].value
        ...
        case "achWrapper2":
            ...
            label.appendChild(labelText);
            resultText = document.createTextNode(
                `${formatToCurrency(el.parentNode.childNodes[1].value).replace(
                    /\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g,
                    ","
                )}`
            );
        ...
        case "batch":
            ...
            label.appendChild(labelText);
            resultText = document.createTextNode(
                `${formatToCurrency(el.value).replace(
                    /\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g,
                    ","
                )}`
            );
...
function resetText(el, item, count, achAmount) {
    ...
    switch (item) {
        case "date":
            ...
            break;
        case `ach${count}`:
            result = `${formatToCurrency(achAmount).replace(
                /\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g,
                ","
            )}`;
            result2 = `ACH from ${el.value}`;
            console.log(
                `within resetText(). case statement =ach${count} result = ${result} result2 = ${result2}`
            );
        ...
        case "from":
            ...
            break;
        case "batch":
            result = `${formatToCurrency(el.value).replace(
                /\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g,
                ","
            )}`;
            break;
...
function createACHinputs(count) {
    ...
    inputAmount.addEventListener("keyup", (event) => {
        ...
        if (valid) {
            console.log(
                "achAmount = " + achAmount + " and achPayee = " + achPayee
            );
    ...
    addACH.addEventListener("click", (event) => {
        ...
        for (member in arrACH) {
            ...
            console.log(member);
            arrACH[member].setAttribute(
                "id",
                `achWrapper${Number(member) + 1}`
            );
            arrACH[member].childNodes[0].setAttribute(
                "id",
                `ia${Number(member) + 1}`
            );
            arrACH[member].childNodes[1].setAttribute(
                "id",
                `if${Number(member) + 1}`
            );
            arrACH[member].childNodes[2].setAttribute(
                "id",
                `action${Number(member) + 1}`
            );
            if (document.getElementById(`ach${eID}`)) {
                document
                    .getElementById(`ach${eID}`)
                    .setAttribute("id", `ach${Number(member) + 1}`);
            }
...
function displayLotReport() {
    ...
    for (var member in inTheShop) {
        ...
        let companyText = document.createTextNode(
            ipc.sendSync("db-get-customer-name", inTheShop[member].customer_ID)
        );
        ...
        let unitText = document.createTextNode(
            `Unit: ${inTheShop[member].unit}`
        );
        ...
        let unitTypeText = document.createTextNode(
            `Unit Type: ${
                inTheShop[member].unit_type ? inTheShop[member].unit_type : ""
            }`
        );
        ...
        let jobTypeText = document.createTextNode(
            `Job Type: ${inTheShop[member].job_type}`
        );
    ...
    for (var member in onTheLot) {
        ...
        let company = document.createElement("div");
        let companyText = document.createTextNode(
            ipc.sendSync("db-get-customer-name", onTheLot[member].customer_ID)
        );
        company.setAttribute("class", "lineItemCompany");
        company.appendChild(companyText);
        lineItem.appendChild(company);
        ...
        let unitTypeText = document.createTextNode(
            `Unit Type: ${
                onTheLot[member].unit_type ? onTheLot[member].unit_type : ""
            }`
        );
        unitType.setAttribute("class", "lineItemSection");
        unitType.appendChild(unitTypeText);
        ...
        let jobType = document.createElement("div");
        let jobTypeText = document.createTextNode(
            `Job Type: ${onTheLot[member].job_type}`
        );
        jobType.appendChild(jobTypeText);
        lineItem.appendChild(jobType);
    ...
    console.log(
        `OnTheLot: ${onTheLot.length} In the Shop: ${inTheShop.length} Completed: ${completed.length} Sch: ${scheduled.length}`
    );
}
...
async function pullNoShows() {
    ...
    for (member in objNoshows) {
        ...
        let name = ipc.sendSync(
            "db-get-customer-name",
            objNoshows[member].customer_ID
        );
        // console.timeEnd('db-get-customer-name')

        let phoneNumber =
            objNoshows[member].number_ID != null &&
            objNoshows[member].number_ID != undefined &&
            objNoshows[member].number_ID != "null"
                ? ipc.sendSync("db-get-phone", objNoshows[member].number_ID)
                : { number: "no number entered" };
        // console.log('phone number'+phoneNumber)
        let objContact =
            phoneNumber.p_contact_ID != null &&
            phoneNumber.p_contact_ID != undefined
                ? ipc.sendSync("db-get-contact", phoneNumber.p_contact_ID)
                : {}; //{'first_name' :'no first name','last_name':'no last name'}
        ...
        let jt = `${objNoshows[member].job_type == "Alignment" ? "an" : "a"} ${
            objNoshows[member].job_type
        } job `;
        let jID = `with a jobID of :${objNoshows[member].job_ID} `;
        let un = `UNIT# [${
            objNoshows[member].unit ? objNoshows[member].unit : "not entered"
        }] `;
        let ut = `${
            objNoshows[member].unit_type
                ? "UNIT TYPE- [" + objNoshows[member].unit_type + "]"
                : ""
        } `;
        let ds = `on ${objNoshows[member].date_scheduled}. `;
        let caller = `${
            objContact.first_name || objContact.last_name ? "set up by " : ""
        } ${fn} ${ln} `;
        let dc = `on ${objNoshows[member].date_called} `;
        let fromNumber = `from number [${phoneNumber.number}]`;
        let notes = `${
            objNoshows[member].notes ? objNoshows[member].notes : ""
        }`;
        let strData = `${n}${jt}${jID}${ds}${caller}${dc}${fromNumber} VEHICLE INFO==> ${un}${ut} NOTES==> ${notes}`;
...
function displayHistory(result) {
    ...
    for (i = 0; i < result.length; i++) {
        if (typeof result[i] === "object") {
            let strDate;
            if (result[i]?.date_completed) {
                strDate = `Completed on ${result[i].date_completed}`;
            } else {
                strDate = `started on ${result[i].date_in} (job before date-completed added to database)`;
            }
            let strData = `${ipc.sendSync(
                "db-get-customer-name",
                result[i].customer_ID
            )} ${result[i].job_type} job on UNIT: ${
                result[i].unit
            } ${strDate}   NOTES: ${result[i].notes}`;
            strData += result[i].no_show
                ? `  <<<NO SHOW on ${result[i].date_scheduled}>>>`
                : "";
            lineItem += `${strData}\n`;
            strForDisplay += `${strData}<br/><br/>`;
...
function fillCustomerDataList() {
    ...
    for (i = 0; i < customerList.length; i++) {
        ...
        newOption.setAttribute(
            "value",
            customerList[i].customer_name.toUpperCase()
        );
