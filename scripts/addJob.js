...
function setData(data) {
    ...
    document.getElementById(
        "rad" + data.time_of_day.toUpperCase()
    ).checked = true;
...
function formatDate(date) {
    ...
    if (pieces[2].length > 4 || pieces[2].length < 2 || pieces[2].length == 3)
        return [false, date];
...
async function clearContacts() {
    $("#txtContacts").find("option").remove().end();

    $("#txtContacts").find("optgroup").remove().end();
}
    ...
    switch (con_ops) {
        case "+ add contact":
            ...
            ipc.send(
                ...
                "add job page",
                chosenCompany,
                isNewCustomer(chosenCompany.toUpperCase())
            );
            break;
        ...
        case "+ add number":
            con_name =
                conOps.options[conOps.selectedIndex].parentElement.label.split(
                    " "
                );
            index = conOps.options[conOps.selectedIndex].index;
            conMeth = "phone";
            con_ID = Number(
                conOps.options[conOps.selectedIndex].parentElement.getAttribute(
                    "contactid"
                )
            );
            console.log(con_ID);
...
function addJob() {
    ...
    //build job object
    objNewJob.customer_ID =
        txtCN.getAttribute("data-cid") != null
            ? txtCN.getAttribute("data-cid")
            : addCompanyNoCantact(decoded.trim().replace(/  +/g, " ")); //addCompanyNoCantact(txtCN.innerHTML.trim().replace(/  +/g, ' '))
    ...
    txtUnitType.value.trim().length
        ? (objNewJob.unit_type = txtUnitType.value)
        : null;
    ...
    document.getElementById("cbCash").checked
