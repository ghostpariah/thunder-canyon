...
function fillSections(users) {
    ...
    for (member in users) {
        ...
        password.addEventListener("change", (event) => {
            ...
            if (!exists) {
                ...
                if (arrChanges[existingObjectIndex].password != users)
                    arrChanges[existingObjectIndex].password =
        ...
        userRole.addEventListener("change", (event) => {
            ...
            if (!exists) {
                ...
                arrChanges[existingObjectIndex].role =
                    event.currentTarget.value;
            }
        });
        ...
        activeUser.addEventListener("change", (event) => {
            ...
            if (!exists) {
                ...
                arrChanges[existingObjectIndex].active =
        ...
        if (users[member].active == "1" || users[member].active == "true")
            document.getElementById(
                `cb${users[member].user_ID}`
            ).checked = true;
        document
            .getElementById(`dr${users[member].user_ID}`)
            .appendChild(roleOption1);
        document
            .getElementById(`dr${users[member].user_ID}`)
            .appendChild(roleOption2);

        if (users[member].role == "admin") {
            document.getElementById(
                `dr${users[member].user_ID}`
            ).selectedIndex = 0;
        } else {
            document.getElementById(
                `dr${users[member].user_ID}`
    ...
    submitButton.setAttribute("class", "-user-submit");
...
function displaySection(args) {
    ...
    for (i = 1; i < buttons.length + 1; i++) {
        document
            ...
            .setAttribute("class", "buttons unchosen");
    ...
    optionStatus < 0
        ? args.setAttribute("class", "buttons unchosen")
        : args.setAttribute("class", "buttons chosen");

    if (args) {
        document.getElementById(`wrapper${chosenOption}`).style.display =
            "block";
    }
}
...
function createUser() {
    ...
    urEl.options[urEl.selectedIndex].text
        ? (userData.role = urEl.options[urEl.selectedIndex].text)
