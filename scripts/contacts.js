...
$(inpFirstname).on({
    keydown: function (event) {
        ...
        let visibility = document.getElementById("firstname_message_container")
            .style.display;
        // if(visibility == 'block'){
    ...
    blur: function () {
        console.log(
            " firstname blur -- contactInfoValid : " + contactInfoValid
        );
    },
});
...
$(inpEmail).on({
    ...
    keyup: function (event) {
        if (emailValid) {
            //document.getElementById('email_message').innerHTML = "";
            document.getElementById("email_message_container").style.display =
                "none";
        }
        emailValid = false;
        const exp = /[^@][@]/;
    ...
    blur: function (event) {
        ...
        if (emailValid === false) {
            ...
            document
                .getElementById("email_message_container")
                .appendChild(createMessageBox("email", "invalid"));
            //document.getElementById("email_message").innerText = "Invalid email";
            document.getElementById("email_message_container").style.display =
                "block";
...
function togglePageView(action) {
    ...
    try {
        ...
        for (i = 0; i < buttonList.length; i++) {
            buttonList[i].setAttribute(
                "class",
                "mainContactButton buttonUnselected"
            );
        }
        ...
        switch (action) {
            case "add":
                ...
                document
                    .getElementById("btnMainAdd")
                    .setAttribute("class", "mainContactButton buttonSelected");

                break;
            case "edit":
                document.getElementById("btnMainEdit").click();
                document
                    .getElementById("btnMainEdit")
                    .setAttribute("class", "mainContactButton buttonSelected");
...
function loadHeader(arg, id) {
    ...
    if (arg == undefined || arg == null)
        return console.log(`arg was undefined line 414 in loadHeader()`);
    //companyID = addCN;
    //companyID = ipc.sendSync('get-customer-ID', arg);
    console.log(
        `companyName is ${arg} and companyID is ${companyID} after guardian clause`
    );
    ...
    let compHeadText = document.createTextNode(arg.toUpperCase());
    ...
    companyHeader.style.display = "block";
    btnAddCompanyContact.setAttribute(
        "class",
        "mainContactButton buttonUnselected"
    );
    ...
    btnEditCompanyContact.setAttribute(
        "class",
        "mainContactButton buttonUnselected"
    );
...
function loadContactsPage(firstInstance) {
    ...
    if (!firstInstance) {
        //customerNameWrapper.style.display ="none"
        console.log(
            "the customer id in loadContactsPage is " + props.customer_ID
        );
        loadHeader(props.customer_name);
    ...
    $("#Customer-choice").on({
        ...
        input: function () {
            ...
            if (
                ...
                var opt = $(
                    'option[value="' + $(this).val().toUpperCase() + '"]'
                );
...
function fillCustomerDataList() {
    ...
    for (i = 0; i < customerList.length; i++) {
        ...
        newOption.setAttribute("value", uNames[i].customer_name.toUpperCase());
...
function validateForm() {
    console.log(
        `${contactInfoValid} ${inpFirstname.value} ${inpPhoneNumber.value} ${inpEmail.value}`
    );
    console.log(inpPhoneNumber.value.length);
    contactInfoValid =
        inpPhoneNumber.value != "" && inpPhoneNumber.value.length >= 14
            ? true
            : inpEmail.value != ""
            ? true
            : false;
    ...
    if (inpFirstname.value == "") {
        ...
        document
            .getElementById("firstname_message_container")
            .appendChild(createMessageBox("fName"));
        document.getElementById("firstname_message_container").style.display =
            "block";
        //document.getElementById('firstname_message').innerHTML = 'First name is required'
    }
    if (
        !contactInfoValid ||
        (inpPhoneNumber.value == "" && inpEmail.value == "")
    ) {
        ...
        document
            .getElementById("phone_message_container")
            .appendChild(createMessageBox("phone"));
        document
            .getElementById("email_message_container")
            .appendChild(createMessageBox("email"));
        //document.getElementById('message').innerHTML = "A valid phone number or email is required"
        document.getElementById("phone_message_container").style.display =
            "block";
        document.getElementById("email_message_container").style.display =
            "block";
    }
...
function checkJobsForItem(obj) {
    ...
    for (member in allJobs) {
        let om =
            obj.searchGroup == "p"
                ? allJobs[member].number_ID
                : allJobs[member].email_ID;
        if (obj.methodID == om) {
...
function fillContacts(cont) {
    ...
    cont.sort((x, y) =>
        x.contact_ID == first ? -1 : y.contact_ID == first ? 1 : 0
    );
    ...
    for (member in cont) {
        ...
        if (member != "primary_contact") {
            ...
            cbPrimary.setAttribute(
                "id",
                `ci${String(cont[member].contact_ID).padStart(5, "0")}`
            );
            ...
            linkEl1.setAttribute(
                "data-contact-id",
                `${cont[member].contact_ID}`
            );
            ...
            linkEl1.addEventListener("click", (event) => {
                ...
                let lnText =
                    event.target.parentNode.parentNode.childNodes[1]
                        .textContent;
                let p = event.target.parentNode.parentNode.id;
                let cust_ID = event.target.parentNode.getAttribute("data-id");
                let cid =
                    event.target.parentNode.getAttribute("data-contact-id");
            ...
            linkEl2.addEventListener("click", (event) => {
                ...
                for (i = 0; i < cont.length; i++) {
                    if (cont[i].contact_ID == contact_ID) {
                        ...
                        for (member in cont[i].phonenumbers) {
                            ...
                            objD.methodID =
                                cont[i].phonenumbers[member].phone_ID;
                            objD.searchGroup = "p";
            ...
            for (n in cont[member].phonenumbers) {
                ...
                pnEditLinkBox.addEventListener("click", (event) => {
                    ...
                    /*
                      ...
                      */
                    if (
                        document.getElementById("editCreatedListItem") != null
                    ) {
                        document
                            .getElementById("editCreatedListItem")
                            .childNodes[2].click();
                ...
                pTrashIcon.setAttribute(
                    "data-customer-id",
                    cont[member].customer_ID
                );
                pTrashIcon.setAttribute(
                    "data-contact-id",
                    cont[member].contact_ID
                ...
                pTrashIcon.setAttribute(
                    "data-number-id",
                    cont[member].phonenumbers[n].phone_ID
                );
                ...
                pnDeleteLinkBox.addEventListener("click", (event) => {
                    ...
                    let objDel = new Object();
                    objDel.companyID =
                        event.target.getAttribute("data-customer-id");
                    objDel.contactID =
                        event.target.getAttribute("data-contact-id");
                    objDel.method = "p";
                    objDel.methodID =
                        event.target.getAttribute("data-number-id");
            ...
            if (cont[member].contact_ID == conID && addCM == "phone") {
                console.log(
                    `input created foradding ${addCM} to contact id ${conID}`
                );
            ...
            for (n in cont[member].emails) {
                ...
                nLi.setAttribute(
                    "id",
                    `e${String(cont[member].emails[n].email_ID).padStart(
                        5,
                        "0"
                    )}`
                );
                ...
                eEditLinkBox.addEventListener("click", (event) => {
                    ...
                    let newListItem = createInput("email", eID, eText, "edit");
                ...
                eDeleteLinkBox.addEventListener("click", (event) => {
                    ...
                    objDel.method = "e";
                    objDel.methodID =
                        event.target.getAttribute("data-email-id");
                    // objDel.companyID = gggp.substr(4);
                    // objDel.contactID = gp.substr(6);
                    // objDel.method = p.substr(0, 1);
                    // objDel.methodID = p.substr(1);
...
let goingToEdit = (jobs, level) => {
    ...
    if (jobs.jobCount > 1) {
        return confirm(
            `${level} is currently connected to active jobs. Click 'OK' to change the job contacts`
        );
    }
    return confirm(
        `${level} is currently connected to an active job. Click 'OK' to change the job contact`
    );
};
    ...
    if (jobs?.onCurrentJob) {
        ...
        if (goingToEditJobContact) {
            ...
            let newWindow = setInterval(() => {
                ...
                c++;
                if (c == arrEditWindowJobs.length) {
                    clearInterval(newWindow);
                }
            }, 400);
        }
        ...
        console.log(
            "jobs in processDeleteDecision is " + JSON.stringify(objContact)
        );
        console.log(jobs);
        if (level == "Contact") {
        ...
        $(txtSection).on({
            ...
            keydown: (event) => {
                if (event.target.getAttribute("data-state") == "closed") {
                    toggleDD(
                        "closed",
                        document.getElementById("Customer-listBox"),
