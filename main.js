...
const logLocation = path.join(__dirname, `data/logs/${y}/${today}/`)
const broadcaster = path.join(__dirname, 'data/logs', 'activityLog.txt')
const errorLog = path.join(__dirname, 'data/logs', 'errorLog.txt')
let objCompanyList
let objList
let win 
...
app.on('ready', ()=>{
    ...
    if (!fs.existsSync(logLocation)){
            ...
            fs.copyFile(broadcaster, logLocation+'activityLog.txt', fs.constants.COPYFILE_EXC, (err)=>{
                if (err) {
                    console.log("Error Found:", err);
                  }
                  else {
                      console.log('File copied to '+logLocation)
                      fs.truncate(broadcaster,()=>{
                          console.log('Activity Log archived and emptied')
                    })
                  }
            });
            fs.copyFile(errorLog, logLocation+'errorLog.txt', fs.constants.COPYFILE_EXC, (err)=>{
                if (err) {
                    console.log("Error Found:", err);
                  }
                  else {
...
ipcMain.on('add-job', (event,args, args2)=>{
    let objData =args
...
fs.watch(broadcaster, (event, filename) => {
    ...
    //         recursive: true
    //     });
    //     fs.open(broadcaster, 'w', function (err, file) {
    //         if (err) throw err;
    //         console.log('File is opened in write mode.');
    //       });
    //       fs.close(broadcaster, (err) => {
    //         if (err)
...
ipcMain.on('update-job',(event,args,args2)=>{
